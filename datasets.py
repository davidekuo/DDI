...
class DDI_DataModule(pl.LightningDataModule):
  ...
  def __init__(self, 
    ...
    self.disease2label = {disease:label for label, disease in enumerate(sorted(annotations['disease'].unique()))}
    # Add 'disease_label' col to metadata with int label for each diagnosis
    self.metadata['disease_label'] = self.metadata['disease'].map(disease2label)
